...
node* avl:: leftrotate(node* n){
        node* new_root = n->right;// the middle node 
...
if (curr->left !=NULL && curr->right!=NULL){
        ...
        curr->right=deleting(k,rightchild);
}}
...
int main(){
    ...
    //deletion
    tree.deleting(6,tree.root);
    cout<<"\n result of deletion \n";
    tree.inorder(tree.root);

    //searching:
    cout<< "\n result of searching\n";
    if (tree.search(tree.root,5)!=NULL){cout<<"key found";}
